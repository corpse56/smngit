...
namespace SummonManager
{
    class DBRemarkSUMMON :DB
    {
        ...
        {
            DA.SelectCommand.CommandText = "select A.ID,IDSUMMON,DOCUMENTNAME," +
                                           " case when DOCUMENTNAME = 'SERIAL' then 'Состав изделия' else " +
                                           " case when DOCUMENTNAME = 'PLANKA' then 'Габаритный чертёж' else " +
                                           " case when DOCUMENTNAME = 'PASSPORT' then 'Конфигурация' else " +
                                           " case when DOCUMENTNAME = 'MANUAL' then 'Схема электрическая' else " +
                                           " case when DOCUMENTNAME = 'PACKINGLIST' then 'Технические требования' else DOCUMENTNAME end end end end end DOCUMENTNAME_RUS" +
